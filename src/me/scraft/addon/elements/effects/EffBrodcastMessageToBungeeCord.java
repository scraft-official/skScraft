...
public class EffBrodcastMessageToBungeeCord extends Effect {
	 
    static {
        Skript.registerEffect(EffBrodcastMessageToBungeeCord.class, "broadcast bungee[cord] message %string%");
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "Effect: broadcast bungee message %string%";
    }
    ...
    if(message.getSingle(event)==null) {
    	...
    	PacketMessageAllPlayers packet = new PacketMessageAllPlayers(message.getSingle(event));
    	packet.send();
    	
    	if (customConfig.get().getBoolean("DEBUG MODE") == true) {
			Bukkit.getLogger().info("§3[§bDEBUG§3] §eskScraft §e» §6Broadcast sent!");
		}
    }	
  }
}
