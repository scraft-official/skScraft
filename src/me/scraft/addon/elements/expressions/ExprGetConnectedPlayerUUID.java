...
public class ExprGetConnectedPlayerUUID extends SimpleExpression<UUID> {
 
   static {
       Skript.registerExpression(ExprGetConnectedPlayerUUID.class, UUID.class, ExpressionType.COMBINED, "[bungee[cord]] uuid of [player] %string%");
   }
   ...
   protected UUID[] get(Event event) {
       ...
       if(isOnline == IsOnlineResult.ONLINE) {
			...
			if (customConfig.get().getBoolean("DEBUG MODE") == true) {
				Bukkit.getLogger().info("§3[§bDEBUG§3] §eskScraft §e» §6UUID of player " + player.getSingle(event) + " is " + info.toString() + "!");
			}
