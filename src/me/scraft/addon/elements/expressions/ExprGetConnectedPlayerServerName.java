...
public class ExprGetConnectedPlayerServerName extends SimpleExpression<String> {
 
   static {
       Skript.registerExpression(ExprGetConnectedPlayerServerName.class, String.class, ExpressionType.COMBINED, "[bungee[cord]] server of [player] %string%");
   }
   ...
   protected String[] get(Event event) {
       ...
       if(isOnline == IsOnlineResult.ONLINE) {
			...
			PacketGetServerByPlayer packet3 = new PacketGetServerByPlayer(info);
			String info2 = (String) packet3.send();
			
			if (customConfig.get().getBoolean("DEBUG MODE") == true) {
				Bukkit.getLogger().info("§3[§bDEBUG§3] §eskScraft §e» §6Server of player " + player.getSingle(event) + " is " + info2 + "!");
			}
			
			return new String[] {info2};
